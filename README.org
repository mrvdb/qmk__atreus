...
*** TODO Mishaps with shifted sequences
:PROPERTIES:
:CREATED:  [2023-01-08 Sun 15:15]
:END:

Shifted key sequences are timing sensitive apparently, perhaps also because my z and / key are the left and right shift key respectively, so they require a hold period.

What happens is not enirely clear yet, but the wrong character gets capitalized.
(mostly the next in the sequence, leaving out the first entirely)
